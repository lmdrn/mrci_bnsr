...
class Package {
		...
		foreach ( $forTranslation as $propertyName => $translationSettings ) {
			...
			if ( $stringValue ) {
				if ( $this->isArray( $stringValue, $translationSettings ) ) {
					$this->registerOptions( $propertyName, $stringValue, $translationSettings );
				} else {
					$this->registerString(
						$stringValue,
						$propertyName,
	...
	public function translateFormSettings( array $data ) {
		...
		foreach ( $forTranslation as $propertyName => $translationSettings ) {
			...
			if ( $stringValue ) {
				if ( $this->isArray( $stringValue, $translationSettings ) ) {
					$data[ $propertyName ] = $this->translateOptions( $stringValue, $propertyName, $translationSettings );
				} else {
	...
	public function registerField( $fieldId, array $data ) {
		...
		foreach ( $forTranslation as $propertyName => $translationSettings ) {
			...
			if ( $stringValue ) {
				if ( $this->isArray( $stringValue, $translationSettings ) ) {
					$this->registerOptions( $fieldId, $stringValue, $translationSettings );
				} else {
					$this->registerString(
						$stringValue,
						$fieldId,
	 ...
	 */
	private function registerOptions( $fieldId, array $options, array $translationSettings ) {
	 ...
	 * @return array
	 */
	private function translateOptions( array $options, $fieldId, array $translationSettings ) {
